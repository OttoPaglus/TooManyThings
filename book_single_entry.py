...
class BookEntryWindow:
    ...
    def build_widgets(self):
        ...
        Label(self.window, text="CIP数据", font=("等线", 20)).grid(row=row, column=0, sticky=W)
        row += 1
        ...
        Label(self.window, text="存放购买信息", font=("等线", 20)).grid(row=row, column=0, sticky=W)
        row += 1
    ...
    def save_book(self):
        ...
        if fields[11]:
            if not os.path.isfile(fields[11]):
